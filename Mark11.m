...
k=2000;             %bersar paket yangdikirimkan
...
% for z=1:2
    ...
    for i=1:n
        if (SN(i).cond==1) && (SN(i).role==0) && (CLheads>0)
            ...
            if SN(i).E>0
                ...
                %            fprintf('node ID transmit= %u \t , energy = %u \n', [i , SN(i).E].');
    ...
    for i=1:nCH
        ...
        if SN(orderPeg(i)).E>0 && SN(orderPeg(i)).cond==1  && (SN(orderPeg(i)).role==1)
            if i<cl_pos
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                if SN(orderPeg(i)).pos==1 && SN(orderPeg(i)).rolePeg==0
                    ETx= Eelec*k + Eamp*k*SN(orderPeg(i)).dis^2;
                    SN(orderPeg(i)).E=SN(orderPeg(i)).E-ETx;
                    energy=energy+ETx;
                end
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                if SN(orderPeg(i)).pos==0 && SN(orderPeg(i)).rolePeg==0
                    ERx=(EDA+Eelec)*k;
...
%                 fprintf('dis1 id= %u \n', [SN(orderPeg(i)).id].');
            ...
            if i>cl_pos
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                if SN(orderPeg(i)).pos==2 && SN(orderPeg(i)).rolePeg==0
...
%                 fprintf('dis1 id= %u \n', [SN(orderPeg(i)).id].');
            ...
            if i==cl_pos
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                if SN(orderPeg(i)).pos==0  && SN(orderPeg(i)).rolePeg==1
                    ERx=(EDA+Eelec)*k*2;
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                if SN(orderPeg(i)).pos==1  && SN(orderPeg(i)).rolePeg==1
                    ERx=(EDA+Eelec)*k;
                    ETx= (EDA+Eelec)*k + Eamp*k*SN(orderPeg(i)).dts^2;
                    SN(orderPeg(i)).E=SN(orderPeg(i)).E-ETx-ERx;
                    energy=energy+ETx+ERx;
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                if  SN(orderPeg(i)).pos==2  && SN(orderPeg(i)).rolePeg==1
                    ERx=(EDA+Eelec)*k;
                    ETx= (EDA+Eelec)*k + Eamp*k*SN(orderPeg(i)).dts^2;
                    SN(orderPeg(i)).E=SN(orderPeg(i)).E-ETx-ERx;
                    energy=energy+ETx+ERx;
...
% title ({'LP'; 'Average Energy consumed by a Node per Transmission';})
% xlabel 'Transmissions';
% ylabel 'Energy ( J )';
% hold on;
