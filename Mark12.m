...
x=0;
y=0;
n=10;               %jumlah node yang di inisialisasi
...
k=10000;             %bersar paket yangdikirimkan
...
while operating_nodes>0
        ...
        if SN(orderPeg(i)).E<=0 && SN(orderPeg(i)).condPeg==1
            ...
            if SN(orderPeg(i)).posPeg==1 && SN(orderPeg(i)).rolePeg==0
                if operating_nodes==1
                    ...
                    t=i;
                    while SN(orderPeg(t)).condPeg==0 && t<n
                        t=t+1;
                    end
                    SN(orderPeg(t)).posPeg=1;
                    SN(orderPeg(t)).prev=0;
                end
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            if SN(orderPeg(i)).posPeg==2 && SN(orderPeg(i)).role==0
                if operating_nodes==1
                    done='OK';
                else
                    t=i;
                    while SN(orderPeg(t)).condPeg==0 && t>1
                        t=t-1;
                    end
                    SN(orderPeg(t)).posPeg=2;
                    SN(orderPeg(t)).closest=0;
                end
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            if SN(orderPeg(i)).posPeg==0 && SN(orderPeg(i)).role==1
                SN(orderPeg(i)).role=0;
                after=i;
                fprintf('after = %u \n', [after].');
                for l=after:nCH
                    if SN(orderPeg(l)).condPeg==1
                        break;
                    end
                end
                bef=i;
                for j=bef:-1:1
                    if SN(orderPeg(j)).condPeg==1
                        break;
                    end
                end
                SN(orderPeg(j)).closest=orderPeg(l);
                SN(orderPeg(l)).prev=orderPeg(j);
                SN(orderPeg(j)).dis=sqrt((SN(orderPeg(l)).x-SN(orderPeg(j)).x)^2 + (SN(orderPeg(l)).y-SN(orderPeg(j)).y)^2);
                SN(orderPeg(l)).dis2=SN(orderPeg(j)).closest;
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            if SN(orderPeg(i)).posPeg==1 && SN(orderPeg(i)).role==1
                SN(orderPeg(i)).role=0;
                t=i;
                while SN(orderPeg(t)).condPeg==0 && t<n
                    t=t+1;
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            if SN(orderPeg(i)).posPeg==2 && SN(orderPeg(i)).role==1
                SN(orderPeg(i)).role=0;
                t=i;
                while SN(orderPeg(t)).condPeg==0 && t>1
                    t=t-1;
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            if SN(orderPeg(i)).posPeg==0 && SN(orderPeg(i)).role==0
                after=i;
                for l=after:nCH
                    if SN(orderPeg(l)).condPeg==1
                        break;
                ...
                SN(orderPeg(l)).prev=orderPeg(j);
                SN(orderPeg(j)).dis=sqrt((SN(orderPeg(l)).x-SN(orderPeg(j)).x)^2 + (SN(orderPeg(l)).y-SN(orderPeg(j)).y)^2);
                SN(orderPeg(l)).dis2=SN(orderPeg(j)).dis;
            end
    ...
    for i=1:n
        if SN(i).E<=0 && SN(i).cond==1       % if nodes energy depletes with transmission
            dead_nodes=dead_nodes +1;
            operating_nodes= operating_nodes - 1;
            SN(i).cond=0;
            SN(i).chid=0;
